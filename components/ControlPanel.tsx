      ...
      <div className="md:col-span-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`
              flex-1 py-3 px-6 rounded-lg font-semibold text-white shadow-lg transition-all flex items-center justify-center gap-2
          ...
          >
            {isProcessing ? (
              <>
                ...
                <span>{stage === 'ANALYZING' ? 'Analyzing...' : 'Reconstructing...'}</span>
              </>
            ) : (
              <>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
                <span>{stage === 'COMPLETED' ? 'Translate Again' : 'Translate'}</span>
              </>
            )}
          </button>
          ...
          {isComplete && (
            <button
              onClick={onDownload}
              className="px-4 py-3 bg-green-600 hover:bg-green-500 text-white rounded-lg shadow-lg transition-all active:scale-95 flex items-center justify-center"
